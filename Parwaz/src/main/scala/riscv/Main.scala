...
class Main extends Module{
	val io = IO(new Bundle {
		...
		when(control.io.MemtoReg === 0.U){
			register32.io.DataWrite := alu.io.out.asSInt
		...
		when(control.io.operand_A_sel === "b10".U){
			alu.io.in1 := Pc.io.pc4
		}.otherwise{
			alu.io.in1 := register32.io.rs1.asUInt
		}
		when(control.io.extend_sel === "b00".U && control.io.operand_B_sel === "b1".U){
			alu.io.in2 := immediate.io.I_Type.asUInt
		}.elsewhen(control.io.extend_sel === "b10".U && control.io.operand_B_sel === "b1".U){
			alu.io.in2 := immediate.io.S_Type.asUInt
		}.elsewhen(control.io.extend_sel === "b01".U && control.io.operand_B_sel === "b1".U){
			alu.io.in2 := immediate.io.U_Type.asUInt
		}.otherwise{
			alu.io.in2 := register32.io.rs2.asUInt

		}		
		
		alu.io.fn := alu_control.io.ALUcont
		//register32.io.DataWrite := alu.io.out
		//io.reg_output := register32.io.rs1
		io.reg_output := alu.io.out.asSInt
