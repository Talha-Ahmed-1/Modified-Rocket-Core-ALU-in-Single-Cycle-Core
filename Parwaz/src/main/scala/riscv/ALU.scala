...
//    val dw = Input(UInt(1.W))
  ...
  io.branch := cmpInverted(io.fn) ^ Mux(cmpEq(io.fn), in1_xor_in2 === UInt(0), slt)
  ...
  val (shamt, shin_r) = (io.in2(4,0), io.in1)
    ...
    /*else {
      ...
      val shin_hi = Mux(dw === DW_64, io.in1(63,32), shin_hi_32)
      val shamt = Cat(io.in2(5) & (dw === DW_64), io.in2(4,0))
  ...
  if (xLen > 32) {
    require(xLen == 64)
    when (dw === DW_32) { io.out := Cat(Fill(32, out(31)), out(31,0)) }
  }
}
